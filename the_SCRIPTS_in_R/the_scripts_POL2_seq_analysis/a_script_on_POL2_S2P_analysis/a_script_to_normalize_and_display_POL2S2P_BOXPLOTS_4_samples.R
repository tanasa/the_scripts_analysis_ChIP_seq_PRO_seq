...
##################################################################################################################
##################################################################################################################
################################################################################################################## 1 isoform (the longest isoform) / gene
##################################################################################################################
...
##################################################################################################################
################################################################################################################## 
################################################################################################################## preparing the file for NORMALIZATION :
...
png(paste(TITLE_HERE, "ALL.genes", "display_after_NORMALIZATION.in.edgeR.png", sep="."))
boxplot(counts.per.m[,1], 
        counts.per.m[,2],
        counts.per.m[,3],
        counts.per.m[,4],
        ylim=LIMITS,
        col="yellow", 
        main=" NORMALIZED COUNTS in edgeR")
dev.off()
...
png(paste(TITLE_HERE, "counts.per.m.z.genes.PR.png", sep="."), height=12, width=10, units="cm", res=800)
...
png(paste(TITLE_HERE, "counts.per.m.z.genes.OG.png", sep="."), height=12, width=10, units="cm", res=800)
...
png(paste(TITLE_HERE, "counts.per.m.z.genes.RAND2.png", sep="."), height=12, width=10, units="cm", res=800)
...
png(paste(TITLE_HERE, "display.the.BOXPLOTS.of.CPM", "png", sep="."), height=12, width=10, units="cm", res=800)
boxplot(genes.PR.simple.NORM$minus, 
        genes.PR.simple.NORM$plus,
        genes.PR.simple.NORM$minus1,
        genes.PR.simple.NORM$plus1,
...
TITLE_HERE="comp:genes.PR:genes.RAND"
COLORS2 = c("red", "blue", "red", "blue")
...
write.table("\n UP_PR :vs: UP_nonPR (OG) # considering the KD \n",
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE) 
...
write.table(paste("wilcoxon_pval:genes_PR:minus:vs:genes_PR:minus1", 
            wilcox.test(genes.PR.simple.NORM$minus, genes.PR.simple.NORM$minus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)

write.table(paste("wilcoxon_pval:genes_PR:plus:vs:genes_PR:plus1", 
            wilcox.test(genes.PR.simple.NORM$plus, genes.PR.simple.NORM$plus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)

write.table(paste("wilcoxon_pval:genes_OG:minus:vs:genes_OG:minus1", 
            wilcox.test(genes.OG.simple.NORM$minus, genes.OG.simple.NORM$minus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)

write.table(paste("wilcoxon_pval:genes_OG:plus:vs:genes_OG:plus1", 
            wilcox.test(genes.OG.simple.NORM$plus, genes.OG.simple.NORM$plus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)
...
write.table("\n UP_PR :vs: RANDOM (r2) for the samples with KD \n",
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE) 
...
write.table(paste("median_ChIP:genes_RAND2:minus1", median(genes.RAND2.simple.NORM$minus1), sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE) 

write.table(paste("median_ChIP:genes_RAND2:plus1", median(genes.RAND2.simple.NORM$plus1), sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)
####
...
write.table(paste("wilcoxon_pval:genes_RAND2:minus1:vs:genes_RAND2:plus1", 
...
write.table(paste("wilcoxon_pval:genes_RAND2:minus:vs:genes_RAND2:minus1", 
            wilcox.test(genes.RAND2.simple.NORM$minus, genes.RAND2.simple.NORM$minus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)

write.table(paste("wilcoxon_pval:genes_RAND2:plus:vs:genes_RAND2:plus1", 
            wilcox.test(genes.RAND2.simple.NORM$plus, genes.RAND2.simple.NORM$plus1)$p.value , sep=" : "),
            file=TITLE_HERE, 
            append=TRUE, quote=FALSE, sep="\t",
            row.names=FALSE, col.names=FALSE)
